...
#include <string>
#include <functional>
std::thread initialize_server(bool const& should_close, std::function<void(std::string const& ip, size_t port)> peer_connected, std::function<void(std::string const& ip, size_t port)> peer_disconnected, std::function<void(std::string)> packet_received, size_t port = 7777);
